import common
from language_models import GPT, HuggingFace, vllmModel
import torch
...
def load_indiv_model(model_name, tensor_parallel_size=None, device=None):
    model_path, template = get_model_path_and_template(model_name)
    if model_name in ["gpt-3.5-turbo", "gpt-4", "gpt-4-32k"]:
        lm = GPT(model_name)
    elif model_name == "vicuna":
        lm = vllmModel(model_path, tensor_parallel_size=tensor_parallel_size)
    else:
...
def get_model_path_and_template(model_name):
    full_model_dict = {
        "gpt-4-turbo": {"path": "gpt-4-turbo", "template": "gpt-4"},
        "gpt-3.5-turbo": {"path": "gpt-3.5-turbo", "template": "gpt-3.5-turbo"},
        "vicuna": {
            "path": "lmsys/vicuna-7b-v1.5",
            "template": "lmsys/vicuna-7b-v1.5",
        },
        "llama-2": {
            "path": "meta-llama/Llama-2-13b-chat-hf",
            "template": "meta-llama/Llama-2-13b-chat-hf",
        },
        "mixtual": {
            "path": "mistralai/Mixtral-8x7B-v0.1",
            "template": "mistralai/Mixtral-8x7B-v0.1",
        },
    }
    path, template = (
